...
function deleteCheckedData() {
	...
	for(i = 0; i < dataRowCheckBoxNodeList.length; i++) {
		...
		if(checkBoxNode.checked) {
			trNode = $(checkBoxNode).parents('[listData="DODataModificationCommitTask"]')[0];
	...
	myShowConfirmMsg(DEFAULT_MSG_CONFIRM_DELETE_DATA, function() {
		setTimeout(function() {
			myAjax({
				...
				dataType: "text",
				data: {
					serviceType: "com.beef.dataorigin.test.ws.service.DODataModificationCommitTaskDataDetailService",
...
function gotoDataDetail(thisNode) {
	...
	var trNode;
	if($(thisNode).attr('listData') == 'DODataModificationCommitTask') {
		trNode = thisNode;
	} else {
		trNode = $(thisNode).parents('[listData="DODataModificationCommitTask"]')[0];
	}
