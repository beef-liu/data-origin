...
(function( window, undefined ) {
	...
	function EasyJsDomUtil() {
		...
		/**
		 * Load content of data xml into assigned dom node
		 * @param args  an object which has properties as below:
		 * args.dataListXml : string in xml format. For example: <List><TestData>...</TestData><TestData>...</TestData>...</List> 
		 * args.dataXmlNodeName: node name of the Data. In example above it is "TestData"
		 * args.dataListDomNode : node where xml loading to
		 * args.dataDomNodeCopy : copy of the dom node which is used to displays content of data
		 * args.domNodeAttrName : Dom节点下的attribute名，用于扫描过滤
		 * args.dataNodeDidLoadFunc : List中的节点数据装载完成后的回调函数，在循环装载Data节点时，每个节点调用一次.
		 * 			格式：dataNodeDidLoadFunc(domNode, index, length)
		 */
		...
		*/
		...
		/**
		 * dataAttributeName: String 属性名
		 * dataXmlNode: String 数据对象对应的XML的根节点
		 */
			...
			if(node.attributes != null) {
				if(isMSIE()){
					for(var i = 0; i < node.attributes.length; i++) {
						if(isStrEqualIgnoreCase(node.attributes[i].name, attributeName)) {
...
//		    }
		...
		*/
		...
		this.setNodeDisabled = function(domNode, depth, attributeName) {
			var rootNode = domNode;
			if(domNode.length != undefined) {
				rootNode = domNode[0];
			}
			...
			traverseDomNodeFilteredByAttributeName(
					...
					function(nodeTmp, depthTmp) {
						...
						setDomNodeDisabled(nodeTmp);
	            	}
			);
		};
		
		function setDomNodeDisabled(node) {
			if(node.tagName.toLowerCase() == "input") {
				if(node.type.toLowerCase() == "radio")
				{
					//visit all sibling
			...
			else if(node.tagName.toLowerCase() == "select") 
			{
				...
				var radioChildren = $(node).find('input:radio');
				if(radioChildren.length > 0) {
					for(var i = 0; i < radioChildren.length; i++) {
						radioChildren[i].disabled = true;
					}
